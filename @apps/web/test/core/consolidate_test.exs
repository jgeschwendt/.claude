defmodule Core.Memory.ConsolidateTest do
  use ExUnit.Case, async: false

  alias Core.Memory
  alias Core.Memory.Consolidate
  ...
  # on the second — so the whole suite exercises Consolidate's claude-free paths.

  @bank "-tmp-project"

  setup do
    root = Path.join(System.tmp_dir!(), "consolidate_test_#{System.unique_integer([:positive])}")
    File.mkdir_p!(root)
  ...
  test "first run_due baselines the bank and never consolidates", %{root: root} do
    seed_bank()
    count = length(Memory.bank_memories(@bank))

    assert Consolidate.run_due() == []

    state_path = Path.join([root, @bank, "_consolidation.json"])
