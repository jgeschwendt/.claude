...
- Every LiveView renders its own full-screen chrome via `Web.UI` (`<.rail>`, `<.ph>` phosphor icons, `<.message>`/`<.block>`) — there is no `Layouts.app` wrapper and no core_components.ex.
- Icons are Phosphor: `<.ph name="…" />`; heroicons and daisyUI are not installed.
- Forms are plain HTML inputs/selects/textareas styled by `assets/css/app.css` — there is no `<.input>` component.
