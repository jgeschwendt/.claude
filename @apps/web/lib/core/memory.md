...
extraction pipeline (extract → judge → commit, all through `Core.Memory` — the single
...
flowchart TB
    ...
    DEL -->|"archive + finalize<br/>(delete-session.sh)"| ARC["@log/archive/&lt;date&gt;/&lt;sid&gt;.jsonl.gz<br/>(recoverable, un-resumable)"]
...
sequenceDiagram
    ...
    S->>S: drain rules/learn-code.md → rules/*.md<br/>(the one judgment task that stays in-session)
...
### One extraction call — `Core.Claude`
...
- browse banks (managed + read-only auto), live-reloading via `Core.Watcher` PubSub
...
recoverable, un-resumable in place). Nothing anywhere is erased outright.
