defmodule Core.Transcripts do
  ...
  `@log/archive/YYYY-MM-DD/` (recoverable, fuel for the daily dream — same contract as
  ...
  def delete_session(project, id) do
    if Core.Store.component?(project) and Core.Store.component?(id) do
  ...
  defp archive_transcript(id, data) do
    dir = Path.join(Core.UserLog.archive_root(), Date.to_iso8601(Date.utc_today()))
