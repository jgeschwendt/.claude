...
- **Endings are total—the system routes them, not you:** endings are reaped after the fact, and a manual `/compact` snapshots the discarded prefix the same way (PostCompact). No ending asks you for a verdict. `/delete` is the one ending that keeps nothing.
