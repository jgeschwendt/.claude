...
Committed session memory lives in per-directory banks under `~/.orrery/memory/<bank>/` (bank = cwd with every non-alphanumeric character → `-`). Pipeline mechanics (the reconcile pass, the pointer queue, the 90-day buffer, curation) are documented in orrery's `lib/orrery/memory.md` (github.com/jgeschwendt/orrery, cloned locally) and the `/dissolve`·`/delete` skills—read those when needed; this file deliberately doesn't restate them, they change faster than it does.
...
- **Write at the time of attention:** the moment a durable memory surfaces, append one `{at, body}` line to this session's pending sidecar, `~/.orrery/memory/.pending/$CLAUDE_CODE_SESSION_ID.jsonl`—`~/.claude/skills/dissolve/scripts/attend-v2.sh "<body>"` does it, and so does a plain file append, so this path never needs a hook. The ending verb adopts the sidecar into the conversation's queue pointer, where extraction reads each line as a pre-extracted candidate. Never defer to session end—not every ending extracts. A note rides its own conversation and is never committed alone; there is no standalone staging file.
- **End deliberately—endings are a total function:** `/dissolve` routes the whole conversation for extraction (`mode: full`), `/delete` routes it for salvage-only triage (`mode: salvage`—instruction-shaped content only, and a valueless transcript is erased), `/delete hard` erases it outright (no pointer, no buffer, unrecoverable). The built-in `/clear` and `/compact` are hook-covered: `/clear` routes the cleared context as `full`, and a manual `/compact` is gated once—route the conversation, then re-issue. `/exit` routes nothing by design; an `/exit`'d conversation stays outside memory, its sidecar waiting for whichever verb eventually routes it.
