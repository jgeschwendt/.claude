...
| Design — _build under constraints_   | Candidate shapes, including the boring one         | Constraint check; failure modes; "what breaks this?"    | Inversion, premortem, MECE                             |
...
Everything in the Ground and Challenge phases runs on one hierarchy. When you need to check a claim, climb as high as the claim's importance demands:
...
- **Subagents for parallel branches**: when branches are genuinely independent and each needs real investigation, spawn a Task subagent per branch with just that branch's question; each returns evidence plus a mini-verdict, and the main chain merges or prunes. Independence is not only speed — a subagent that never saw your favorite hypothesis evaluates evidence unanchored. And read what comes back through the inheritance lens: a mini-verdict is a claim carrying its own assumptions — merge its assumption list, not just its conclusion.
- **Research-grade evidence**: when a load-bearing claim needs multi-source web research rather than a one-command check, invoke the `research` skill instead of ad-hoc searching. Its ledger statuses arrive pre-graded: `established` (2+ independent sources) enters your ledger as an externally-anchored FACT; `reported` enters as an attributed BELIEF, not a FACT.
- **Subagents as red team**: for the second-opinion move in the gate, the prompt discipline matters — give the subagent the observations and the question, _not_ your conclusion or your chain, and ask open questions rather than "is my answer right?". Prefer one independent pass and a comparison over multi-round debate between agents; if a genuinely different model or configuration is available for the second pass, the added diversity helps. If it converges independently, that is real corroboration. If you can't resist hinting, the exercise is theater.
...
When ground truth eventually arrives — the fix worked or didn't, the decision aged well or badly — spend one `postmortem` thought: what did the chain believe at thought N that turned out wrong, and what signal was already visible then? Calibration only improves with feedback, and the feedback compounds: in a four-year forecasting tournament, a single hour of debiasing training measurably improved accuracy for a full year afterward. The trail you kept makes your own feedback loop nearly free.
