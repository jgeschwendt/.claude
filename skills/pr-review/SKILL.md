...
3. **Present the result** — verdict first, then the finding deltas grouped by status:
   **addressed** (what the last push fixed) · **stands** (still open) · **new** (this
   round's). Then offer `--post`; never pass it unprompted.

**The session model never reviews the diff itself.** The script's pinned call does — that is
CLAUDE.md's premium-models-plan-and-review rule applied to judgement work: the reviewer is
`PR_REVIEW_MODEL` (default `opus`), and this session relays its output, answers questions
about it, and helps decide what to act on. Reading the diff yourself to second-guess a
finding defeats the standing conversation, which is the only thing that knows what round 1
said.
...
The design is orrery's memory system, applied to a PR instead of a session — orrery
`lib/orrery/memory.md` (prose) and `docs/memory-atlas.html` (the same mechanics as plates).
Four shapes are inherited wholesale:

- **Append-only journal, derived state.** `ledger.jsonl` is the truth; `state.json` is a
  cache of its tail, rebuildable and never authoritative — exactly the pointer queue's
...
| `PR_REVIEW_DRY`         | `0`                 | `1` prints the round, session flag, delta source and full prompt to stdout, then exits — no call, no state |
...
- **Merged or closed PRs stay reviewable.** The script warns on stderr and continues; the
  findings just apply to a landed diff.
