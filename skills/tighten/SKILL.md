...
- **Null-prompt test**: would the current model already do this unprompted? Suspects by hit rate — anti-laziness pushes · verify-before-done and self-verification scaffolding · forced progress updates · reasoning-echo instructions · blanket tool defaults · enumerations of sibling behaviors one principle now covers. Adjudicate at rung 3: remove and re-run. The fix for stale scaffolding is deletion, not rewording. A § Keep-list product-consistency instruction survives a null-prompt pass by construction — its gap is the human's.
